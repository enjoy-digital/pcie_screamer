...
struct usb_ft60x {
	...
	struct urb		*int_in_urb;		/* the int to read data with */
	...
	wait_queue_head_t	bulk_out_wait;		/* to wait for an ongoing read */
...
static int ft60x_set_config(struct ft60x_device *ft, struct ft60x_config *ft60x_cfg);
...
static void ft60x_delete(struct kref *kref)
{
	...
	if(dev->ft->n_interface == 0) {
		printk(KERN_INFO "Last interface, so remving dev\n");
...
static unsigned int ft60x_poll(struct file *file, poll_table *wait)
{
	...
	char notif;
        ...
        dev = file->private_data;
	//printk(KERN_INFO "IN_FUCNTION %s\n", __func__);
        if (!dev->interface)
	...
	if(((dev->bulk_in_filled - dev->bulk_in_copied) && dev->done_reading) || dev->got_int) {
		//printk("POLL %lu %d %d\n", dev->bulk_in_filled, dev->done_reading, dev->got_int);
		dev->got_int=0;
...
static void ft60x_int_callback(struct urb *urb)
{
	...
	//struct ft60x_device *ft = urb->context;	
	int retval;
			...
			if(tmpdev) {
				if(tmpdev->bulk_in_endpointAddr == irqresp->ep) {
					...
					if(ongoing_read && !tmpdev->waiting_int) {
						printk(KERN_INFO "status %d %d\n", tmpdev->bulk_in_urb->status, tmpdev->bulk_in_urb->actual_length);
...
//printk(KERN_INFO "IN_FUCNTION %s\n", __func__);
	...
	if(notif) {
		usb_fill_bulk_urb(dev->bulk_in_urb,
				  ...
				  dev->bulk_in_buffer,
				  min(dev->bulk_in_size * 128, dev->len_to_read),
				  ft60x_read_bulk_callback,
				  dev);
	} else {
		usb_fill_bulk_urb(dev->bulk_in_urb,
				  dev->udev,
			  ...
			  size_t count, loff_t *ppos)
{
	...
	usb_fill_bulk_urb(urb, dev->udev,
			  ...
			  buf, writesize, ft60x_write_bulk_callback, dev);
...
static long ft60x_ioctl(struct file *file , unsigned int cmd, unsigned long arg)
{
	...
	case 0:
		...
		break;
	case 1:
		if (copy_from_user(&cfg, (unsigned char*)arg,  sizeof(struct ft60x_config))) {
			retval = -EFAULT;
			goto exit;
		}
...
static int ft60x_set_config(struct ft60x_device *ft, struct ft60x_config *ft60x_cfg)
{
	...
	printk(KERN_INFO "IN FUNCTION SETCONF\n");
	/* Make sure we are not sneding the same thing because it'd disconnect us */
        if(!memcmp(ft60x_cfg, &ft->ft60x_cfg, sizeof(struct ft60x_config))){
		return 0;
	}

	printk(KERN_INFO "CHANGING CONF\n");
	...
	ret = usb_control_msg(ft->udev, usb_sndctrlpipe(ft->udev, 0),
			      ...
			      cfg, sizeof(struct ft60x_config), USB_CTRL_SET_TIMEOUT);
				      ...
				      struct usb_host_interface *iface_desc)
{
	...
	for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
		...
		if (!dev->bulk_in_endpointAddr &&
			...
			dev->bulk_in_buffer = kmalloc(buffer_size*128, GFP_KERNEL);
			if (!dev->bulk_in_buffer) {
				retval = -ENOMEM;
				goto error;
			}
				      ...
				      struct usb_host_interface *iface_desc)
{
	...
	for (i = 0; i < iface_desc->desc.bNumEndpoints; ++i) {
		...
		if (!dev->int_in_endpointAddr &&
			...
			dev->int_in_buffer = usb_alloc_coherent(
				dev->udev,
			...
			usb_fill_int_urb(dev->int_in_urb, dev->udev,
					 pipe, dev->int_in_buffer,  maxp, ft60x_int_callback,
					 dev , endpoint->bInterval);
...
static int ft60x_alloc_device(struct usb_device *device, struct ft60x_device **ftout)
{
...
static int ft60x_find_device(struct usb_device *dev, struct ft60x_device **ftout)
{
	struct ft60x_device *ft=NULL;
	int retval =0;
